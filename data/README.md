...
├── processed/                  # PyTorch timesteps
│   └── timestep_*.pt          # Snapshot tensors
├── features/                   # Full feature tables
│   ├── all_features_raw.csv    # 24 features (raw)
│   ├── all_features_processed.csv # 24 features (normalized)
...
```python
import pandas as pd
values = pd.read_csv('data/raw/values.csv', index_col=[0, 1])
