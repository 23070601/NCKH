...
def check_existing_predictions(analysis_dir='data/analysis'):
    """Check if base predictions already exist."""
    files = sorted(Path(analysis_dir).glob("predictions_*.csv"))
    # Exclude improved_lag files
...
def load_test_data():
    """Load test data from pytorch files."""
    try:
        import torch
    except ImportError:
    ...
    # Use last 113 files as test (15% of 747)
...
def prepare_test_features(test_data):
    """Prepare features from test data with lag features."""
    X_list = []
    y_list = []
    ...
    for i, data in enumerate(test_data):
        try:
            ...
            # Add lag features (like training does)
            lag_features = []
...
def load_selector():
    """Load feature selector from training."""
    selector_files = sorted(glob.glob('data/analysis/quick_improvement/feature_selector_*.pkl'))
    if not selector_files:
        raise FileNotFoundError("No feature selector found. Run train_models.py first.")
    return joblib.load(selector_files[-1])


def generate_base_predictions():
    """Generate base predictions using trained models."""
    print("=" * 70)
    ...
    # Load and use trained ensemble model
    regressor_files = sorted(glob.glob('data/analysis/quick_improvement/improved_regressor_*.pkl'))
    if not regressor_files:
        raise FileNotFoundError("No regressor found. Run train_models.py first.")
    ...
    # Create DataFrame with required columns for lag-based improvement
    n_samples = len(y_test)
    dates = pd.date_range(start='2025-01-01', periods=n_samples, freq='D')
    symbols = [f'STOCK_{i % 98}' for i in range(n_samples)]  # Cycle through 98 stocks
    ...
    df_base = pd.DataFrame({
        ...
        'Pred_Vol': y_pred,
        'DailyLogReturn': np.random.randn(n_samples) * 0.01,  # Placeholder for lag features
...
if __name__ == '__main__':
    ...
    try:
        ...
        if exists and not args.force:
            ...
            print(f"  Use --force flag to recreate: python create_base_predictions.py --force")
        else:
