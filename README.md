...
> 📌 **ALGORITHM COMPARISON FRAMEWORK** 🆕: See [docs/ALGORITHM_COMPARISON_GUIDE.md](docs/ALGORITHM_COMPARISON_GUIDE.md) for the testing strategy comparing Baseline (Historical Mean/ARIMA), ML (Random Forest), and DL (LSTM/GRU) models to find the optimal algorithm for your research!
...
├── data/                           # Data files
│   ├── fdi_stocks_list.csv        # Input: 100 FDI stocks
│   ├── fundamentals.csv           # Output: Financial metrics
│   ├── fdi_prices.csv             # Output: Daily stock prices
│   └── stock_correlation.npy      # Output: Stock correlation matrix
├── notebooks/                      # Jupyter notebooks (analysis pipeline)
│   ├── 0_data_collection.ipynb    # Collect & structure data
│   ├── 1_data_preparation.ipynb   # Calculate volatility metrics
│   └── 2_model_comparison.ipynb   # Compare algorithms & recommend optimal
├── src/                            # Source code modules
│   ├── VNStocks.py                # Data collection and preprocessing
│   ├── utils.py                   # Helper functions
│   └── model_comparison.py        # Algorithm implementations
├── docs/                           # Documentation
│   ├── ALGORITHM_COMPARISON_GUIDE.md  # Strategy & methodology
...
**INPUT Files (Not regenerated):**
- `fdi_stocks_list.csv` - 100 Vietnamese FDI companies (source list)

**OUTPUT Files (Generated by notebook 0):**
- `fundamentals.csv` - Financial metrics for each stock (PE, ROE, etc.)
- `fdi_prices.csv` - Daily closing prices aligned by date
- `stock_correlation.npy` - Correlation-based adjacency matrix

**Note:** Files are named for clarity (fdi_* prefix indicates FDI-specific data). The file `stocks.csv` (duplicate of input list) has been removed. Use `fdi_stocks_list.csv` as the source.
...
- Engineer features for multi-algorithm comparison
- Prepare data for Baseline, ML, and Deep Learning models
