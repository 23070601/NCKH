...
def check_existing_improved_predictions(analysis_dir=Path('data/results/predictions')):
...
def run_backtest_on_predictions(predictions: np.ndarray, y_true: np.ndarray, analysis_dir: Path):
    ...
    backtest_dir = Path('data/results/backtest')
...
def cleanup_old_results():
    import glob
    analysis_dir = Path('data/results/predictions')
    ...
    old_summaries = list(Path('data/results/backtest').glob("backtest_summary_*.json"))
...
def main():
    ...
    if exists and not args.force:
        ...
        print("  Use --force flag to regenerate: python pipeline/06_generate_predictions.py --force")
        return
